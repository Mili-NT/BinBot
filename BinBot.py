...
def config(isManual):
    ...
    if isManual:
        ...
        while True:
            ...
            workpath = syspath[0] if workpath.lower() == 'curdir' else workpath = workpath
            if path.isdir(workpath):
                lib.print_success("Valid Path...")
                workpath = f'{workpath}/' if workpath.endswith('\\') or workpath.endswith('/') else workpath = workpath
                break
            else:
        ...
        while True:
            try:
                ...
                limiter = 5 if limiter == "" else limiter = limiter
                cooldown = 1200 if cooldown == "" else cooldown = cooldown
                break
            except ValueError:
